...
function Movies() {
  ...
  return (
    <>
      <FilterDiv>
        <SelectDiv>
          <Select label='Wybierz kategorie' name='genre' ref={inputGenreRef} options={genres} />
        </SelectDiv>

        <div style={{ width: '150px', marginLeft: '5px' }}>
          <Button label='Szukaj' color='primary' onClick={() => handleSearch()} />
        </div>
      </FilterDiv>
      ...
      <SeeMoreDiv>
        ...
        <Button onClick={() => fetchMovies(selectedGenre)} label='Zobacz więcej..' />
      </SeeMoreDiv>
    </>
  );
}
