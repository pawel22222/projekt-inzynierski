...
function FilterStats({ setSelectedGenre, setAgeRanges, errorFilter, setErrorFilter, generatePDF }) {
  const genres = [
    'Akcja',
    'Przygodowy',
    'Animowany',
  ...
  function getRanges(rangeCounter) {
    return [...Array(rangeCounter)].map((el, i) => {
      return {
  ...
  return (
    <FilterDiv>
      <div>
        <Select lable='Wybierz kategorie: ' name='genre' ref={inputGenreRef} options={genres} />
      </div>

      <div style={{ display: 'flex', width: '300px', padding: '5px 0' }}>
        <Button label='Dodaj przedział' color='green' onClick={() => handleAddRange()} />
        <Button label='Usuń przedział' color='danger' onClick={() => handleRemoveRange()} />
      </div>
      ...
      <div className='d-flex my-2' style={{ width: '400px' }}>
        <Button label='Generuj tabelę' color='primary' onClick={handleSubmit} />

        <Button label='Pobierz PDF' color='secondary' onClick={() => generatePDF()} />
      </div>

      {errorFilter && <Alert type='danger' header={errorFilter.header} desc={errorFilter.desc} />}
    </FilterDiv>
  );
}
